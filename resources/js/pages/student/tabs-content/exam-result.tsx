...
const ExamResult = () => {
   ...
   const attemptScore =
      Number(attempt.total_marks) > 0 ? Math.round((Number(attempt.obtained_marks) / Number(attempt.total_marks)) * 100 * 100) / 100 : 0;
         ...
         <Card className={`border-2 ${isPassed ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50'}`}>
            <CardContent className="p-6">
               <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                  <div className="text-center">
                     <h6 className={`mb-2 text-4xl font-bold ${isPassed ? 'text-green-600' : 'text-red-600'}`}>{attemptScore}%</h6>
