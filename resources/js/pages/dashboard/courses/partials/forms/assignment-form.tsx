...
const AssignmentForm = ({ title, assignment, handler }: Props) => {
   ...
   const handleSubmit = (e: React.FormEvent) => {
      ...
      // Create submission payload with formatted dates
      const submissionData = {
         ...
         deadline: data.deadline ? formatDateForLaravel(data.deadline) : '',
         late_submission: data.late_submission,
         late_total_mark: data.late_total_mark,
         late_deadline: data.late_deadline ? formatDateForLaravel(data.late_deadline) : '',
      };

      if (assignment) {
         router.put(route('assignment.update', assignment.id), submissionData, {
            onSuccess: () => {
               reset();
               setOpen(false);
            },
         ...
         router.post(route('assignment.store'), submissionData, {
            onSuccess: () => {
               reset();
               setOpen(false);
            },
